    ...
    def run(self):#重载Thread的run
        ...
        while True:
            if self.handshaken == False:
                ...
                self.buffer += bytes.decode(self.conn.recv(10240))
                if self.buffer.find('\r\n\r\n') != -1:
                    ...
                    handshake = "HTTP/1.1 101 Switching Protocols\r\n" \
                                "Upgrade: websocket\r\n" \
                                "Connection: Upgrade\r\n"
                    if headers.has_key(u"Sec-WebSocket-Protocol"):
                        handshake=handshake+"Sec-WebSocket-Protocol: "+headers[u"Sec-WebSocket-Protocol"]+"\r\n"
                    handshake=handshake+"Sec-WebSocket-Accept: "+bytes.decode(token)+"\r\n"\
...
class WebSocketServer(threading.Thread):
    ...
    def sendMessage(self,message):
        message_utf_8 = message.encode('utf-8')
        for key,connection in self.connectionlist.items():
            back_str = []
            back_str.append('\x81')
            data_length = len(message_utf_8)
    ...
    def run(self):
        ...
        while True:
            ...
            self.connectionlist['connection'+str(i)]=connection
            connectionlist['connection' + str(i)] = connection
            i = i + 1
class MySocket(threading.Thread):
    def __init__(self,conn,index,name,remote, server):
        threading.Thread.__init__(self)
        self.conn = conn
        self.index = index
        self.name = name
    ...
    def run(self):#重载Thread的run
        while True:
            try:
                buffer=self.conn.recv(10240)
            except socket.error,e:
                break
            if(len(buffer)>0):
                self.server.sendMessage(buffer)



class SocketServer(threading.Thread):
    def __init__(self,port,webss):
        threading.Thread.__init__(self)
        self.socket = None
        self.port=port
        self.connectionlist = {}
    ...
    def sendMessage(self,message):
        message_utf_8 = message.encode('utf-8')
        self.webss.sendMessage(message_utf_8)
    def run(self):
        print('WebSocketServer Start!')
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket.setsockopt(socket.SOL_SOCKET,socket.SO_REUSEADDR,1)
        ...
        while True:
            connection, address = self.socket.accept()
            username=address[0]
            newSocket = MySocket(connection,i,username,address,self.webss)
            newSocket.start() #开始线程,执行run函数
            self.connectionlist['connection'+str(i)]=connection
            i = i + 1
...
if __name__ == "__main__":
    ...
    g_code_length = 0
    g_header_length = 0
    webserver = WebSocketServer(9001)
    webserver.start()
    socketserver=SocketServer(9002,webserver)
    socketserver.start()
